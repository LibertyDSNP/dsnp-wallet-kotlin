package com.unfinished.data.signer.sign
...
fun SignatureWrapperParcel(signatureWrapper: SignatureWrapper): SignatureWrapperParcel {
    return with(signatureWrapper) {
        when (this) {
            is SignatureWrapper.Ed25519 -> SignatureWrapperParcel.Ed25519(
                signature
            )
            is SignatureWrapper.Sr25519 -> SignatureWrapperParcel.Sr25519(
                signature
            )
            is SignatureWrapper.Ecdsa -> SignatureWrapperParcel.Ecdsa(
                v,
                r,
                s
            )
